...
Routing priority is explicit `model` > `agentType` model > `tier` > phase model > metadata model > inherited main model (when the `inheritMainModel` setting is on) > implicit `medium` > session default. Higher priority means selection, not "try this then fall back to the next selector." Avoid specifying competing selectors unless deliberately overriding a lower-priority default. The inherit route needs no tier config; an unavailable inherited model degrades to the settings default with a one-time warning rather than throwing.

Unavailability is asymmetric by design. An explicit `model`, `agentType` model, `tier`, or phase model that resolves to a model the registry doesn't have throws — it never silently runs a different model instead. A tier failure names its source (for example: `tier "big" from model-tiers.json resolves to "deadprov/x", which is not available`), so the mistake is traceable back to the config that caused it. Only the implicit route an untagged agent falls into — the implicit default `medium` tier, or the inherited main model when the `inheritMainModel` setting is on, when the script requested no `model`, `tier`, `agentType`, or phase model — degrades to the session default when it is unavailable, since that agent never asked for that specific model; the degrade still logs a one-time warning into the run so it stays discoverable instead of silent.
